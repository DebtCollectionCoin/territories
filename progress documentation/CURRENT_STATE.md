...
| D — Ktor server skeleton + Postgres | v0.6.0 | 🟡 in progress — `:protocol` (KMP), `:server` (JVM), and `:net` (KMP WebSocket client) modules. WebSocket `/ws` with Welcome/CreateLobby/JoinLobby/SubmitMove/MoveApplied/MoveRejected/GameStarted/GameEnded/Resign. 2-player end-to-end + JVM round-trip tests pass. JS compile passes for `:net`. Postgres + auth + ranked-rating + reconnect deferred |
